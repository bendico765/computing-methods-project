...
This project contains the implementation of U-net based segmentation algorithm for the automatic segmentation of masses in mammography. 
## Pipeline
### Preprocessing
...
### The model
Model is a Unet, slightly different from the original paper implementation since this implementation does not use cropping and zero padding, so it preserves spatial dimensions between the input and the output.
...
[--epochs EPOCHS] [--patience PATIENCE] [--min-delta MIN_DELTA] [--random-state RANDOM_STATE]
